...
void setup() {
  ...
  while(!(iot.available())){
    if(digitalRead(23)){
      lcd.clear();
      Serial.println("connect to dack hotspot");
      lcd.setCursor(0,0);
      lcd.print("IF THIS MESSAGE APPEARS: ");
      lcd.setCursor(0,1);
      lcd.print("CONNECT -SchoolieCam");
      lcd.setCursor(0,2);
      lcd.print("PASSWORD-admin123");
      lcd.setCursor(0,3);
      lcd.print("RESET PASS,RESTART..");
    }
  }
  if(iot.available()){
    String initresponse=iot.readString();
    Serial.println(initresponse);
    if(initresponse){
      lcd.clear();
      lcd.setCursor(0,1);
