...
void stdadd() {
  lcd.setCursor(0,0);
  Serial.println("place the id card");
  lcd.print("place the id for adding");
     if (!rfid.PICC_IsNewCardPresent() || !rfid.PICC_ReadCardSerial())
    return;
...
Serial.print("this is concated file name: ");Serial.println(strID);
  ...
  lcd.clear();
 lcd.setCursor(4,0);
 lcd.print("id is placed");
 lcd.setCursor(0,1);
 lcd.print("connect to HC-05 BLE");
...
Serial.print("the conveerted char is :");Serial.println(asd);
  ...
  if(SD.exists(strID))
  {
    ...
    lcd.clear();lcd.setCursor(5,1);lcd.print("data saved");delay(2000);lcd.clear();
    File dataFile;
    dataFile=SD.open(strID,FILE_READ);Serial.println("file opened in reading mode");
    toprint=NULL;leni=0;iotname=NULL;
   while(((toprint=dataFile.read())!= -1)&&leni<8){
    Serial.print(toprint);iotname[leni++]=toprint;
  ...
  {
    lcd.clear();lcd.setCursor(5,1);lcd.print("saving failed");delay(2000);lcd.clear();
